...
{
    public partial class find_bangla_number : System.Web.UI.Page
    {
        private Function function;

        public find_bangla_number()
        {
        ...
        private void Load()
        {
            findId.Value = function.IsExist($@"SELECT TOP 1 FindId FROM FindAnswer WHERE LangType=N'Bangla' AND QuestionType=N'Number' ORDER BY NEWID()");
            AnswerId.Value = function.IsExist($@"SELECT AnswerPic FROM FindAnswer Where FindId=N'{findId.Value}'");
            lblQuestion.Text = function.IsExist($@"SELECT Question FROM FindAnswer Where FindId=N'{findId.Value}'");
            audioLearn.Src = function.IsExist($@"SELECT Audio FROM FindAnswer Where FindId=N'{findId.Value}'");
            function.LoadDataList(dataOptions, $@"SELECT Picture FROM AnswerPic WHERE FindId=N'{findId.Value}' ORDER BY NEWID()");
