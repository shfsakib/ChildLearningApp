...
namespace ChildLearningApp.app
{
    public partial class bangla_vowel : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                LoadData();
                audioWord.Src = function.IsExist($@"SELECT TOP 1 WordAudio FROM LetterInfo WHERE LetterType=N'Bangla Vowel' ORDER BY LetterId ASC");

            }
        }
        private void LoadData()
        {
            function.LoadDataList(alphabetData, $@"SELECT LETTER FROM LetterInfo WHERE LetterType=N'Bangla Vowel' ORDER BY LetterId ASC");
            imgWord.ImageUrl = function.IsExist($@"SELECT TOP 1 WordPicture FROM LetterInfo WHERE LetterType=N'Bangla Vowel' ORDER BY LetterId ASC");
        ...
        protected void btnLetter_OnClick(object sender, EventArgs e)
        {
            ...
            imgWord.ImageUrl = function.IsExist($@"SELECT TOP 1 WordPicture FROM LetterInfo WHERE LetterType=N'Bangla Vowel' AND Letter=N'{letter.Value}' ORDER BY LetterId ASC");
        ...
        protected void ImageButton1_OnClick(object sender, ImageClickEventArgs e)
        {
            ...
            imgWord.ImageUrl = function.IsExist($@"SELECT TOP 1 WordPicture FROM LetterInfo WHERE LetterType=N'Bangla Vowel' AND Word=N'{word.Value}' ORDER BY LetterId ASC");
            audioWord.Src = function.IsExist($@"SELECT TOP 1 WordAudio FROM LetterInfo WHERE LetterType=N'Bangla Vowel' AND Word=N'{word.Value}' ORDER BY LetterId ASC");
